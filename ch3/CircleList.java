/**
 *  Circulary linked list with node of type Node storing strings.
 *  Simulate of Duck, Duck, Goose with a circularly linked list 
 *  
 * @author hua.zhang
 *
 */
		...
		for (int i =0; i< N; i++) {	 // play Duck, Duck, Goose N times
			...
			if (rand.nextBoolean()) {
				...
				C.add(it); 		//  The "it"  person will be it again in next round
				 ...
				 C.add(it);     // addd who's "it" back at the goose's place
